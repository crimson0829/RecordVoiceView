...
import com.crimson.baidu_asr_library.baiduasr.util.FileUtil;
...
public class CommonRecogParams {
    ...
    protected void initSamplePath(Context context) {
        ...
        if (!FileUtil.makeDir(samplePath)) {
            samplePath = context.getApplicationContext().getExternalFilesDir(sampleDir).getAbsolutePath();
            if (!FileUtil.makeDir(samplePath)) {
...
//            map.put(SpeechConstant.SOUND_CANCEL, R.raw.bdspeech_recognition_cancel);
        ...
        if (sp.getBoolean("_outfile", false)) { // 保存录音文件
            ...
            Logger.error(TAG, "语音录音文件将保存在：" + samplePath + "/outfile.pcm");
        }
